   ...
   updateStartsWithTextOptions() {
    ...
    //   adding arabic chars to filter by browse
      this.arabicChars=['أ','ا','إ','آ','ب','ت','ث','ج','ح','خ','د','ذ','ر','ز','س','ش','ص','ض','ط','ظ','ع','غ','ف','ق','ك','ل','م','ن','ه','و', 'ي','ء' ];
            this.startsWithOptions =(typeof window === 'object' && hasValue(window.localStorage)) && window.localStorage.getItem('selectedLangCode') === 'ar' ? ['0-9',...this.arabicChars ,...'ABCDEFGHIJKLMNOPQRSTUVWXYZ'.split('')] :['0-9', ...'ABCDEFGHIJKLMNOPQRSTUVWXYZ            '.split('') ,...this.arabicChars];  //kware-edit
    }
